...
在这个基础上，再加上个方便操作的 GUI 就好了，于是首先想到的选择就是 `electron`。
...
但如今 `electron` 动不动就上百MB的大小，用来开发这样的小工具实在是不划算。而且在自己另一台破电脑上，`electron` 启动时间动不动就是十几秒，反应也不是很灵敏，达不到小而快的目标效果。

除了 `electron` 之外，是否还有其它的 `node.js` GUI 应用开发方法呢？

于是找了找其它 `node.js` 的 GUI 方案，大家有兴趣的话可以关注一下：
